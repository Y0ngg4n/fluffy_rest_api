...
pub async fn update_text_item(session_arc: &Arc<Session>, text_item: NewUpdateTextItem) -> Result<(), Box<dyn Error>>  {
    ...
    session
        .query(
            "INSERT INTO fluffy_board.toolbar_options_text_item (owner, color_presets, stroke_width, selected_color, selected_cap) VALUES (?, ?, ?, ?, ?);",
            (text_item.owner, text_item.color_presets, text_item.stroke_width, text_item.selected_color, text_item.selected_cap),
        ).await?;
    Ok(())
}